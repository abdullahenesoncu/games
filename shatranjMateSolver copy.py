...
class DFSAI:
    ...
    def findBestMove(self, boardRepr, mateExpectedColor, states):
        _, bestMove = self.dfs(boardRepr, self.maxDepth, boardRepr.split()[ 1 ], mateExpectedColor, states)
        return bestMove
    
    def dfs(self, boardRepr, depth, color, mateExpectedColor, states):
        self.cnt += 1
        if Shatranj.isGameOver(boardRepr):
            return self.heuristic( boardRepr ) + depth * ( 1 if color == 'w' else -1 ), None
        if depth == 0:
        ...
        if color == 'w':
            ...
            for nextRepr, move in possibleMoves:
                ...
                next, _ = self.dfs(nextRepr, depth - 1, 'b', mateExpectedColor, states)
                states.pop()
                if next > maxNext:
                    maxNext = next
                    bestMove = move
            ...
            for nextRepr, move in possibleMoves:
                states.append( nextRepr )
                next, _ = self.dfs(nextRepr, depth - 1, 'w', mateExpectedColor, states)
                states.pop()
                if next < minNext:
                    minNext = next
                    bestMove = move
            return minNext, bestMove

    def heuristic(self, boardRepr):
        return Shatranj.getScore(boardRepr)
